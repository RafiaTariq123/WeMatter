...
function ChatSidebar({ user }) {
  ...
  // Use the filtered users returned from the backend
  ...
  const accountAndLogoutButtons = (
    ...
    >
      <Button
        component={Link}
        to={user?.role === "psychologist" ? "/clinician/dashboard/accountInfo" : "/dashboard/accountInfo"}
        variant="contained"
        sx={{
  ...
  if (isLoading) return <Typography sx={{ p: 2 }}>Loading conversations...</Typography>;
  if (isError) return <Typography sx={{ p: 2 }}>Error loading conversations.</Typography>;
  
  if (users.length === 0) {
    return (
      <Box sx={{ p: 2, textAlign: 'center' }}>
        <Typography>No conversations available at the moment.</Typography>
      </Box>
    );
  }
