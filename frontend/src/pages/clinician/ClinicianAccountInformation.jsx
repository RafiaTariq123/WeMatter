...
function AccountInformation() {
  ...
  // Check if psychologist is pending or rejected - if so, enable editing by default
  const isPendingOrRejected = psychologist?.verificationStatus === 'pending' || psychologist?.verificationStatus === 'rejected';
  const [isDisabled, setIsDisabled] = useState(!isPendingOrRejected);
  ...
  useEffect(() => {
    ...
    // Update disabled state when verification status changes
    if (psychologist?.verificationStatus) {
      const shouldDisable = psychologist.verificationStatus === 'approved';
  ...
  const {
    ...
    onSubmit: async (values, errors) => {
      try {
        ...
        toast.success(
          isPendingOrRejected
            ? "Profile submitted successfully! Your application is now under review."
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box>
        {/* Status Message for Pending/Rejected Psychologists */}
        {isPendingOrRejected && (
          <Box sx={{ mb: 3, p: 2, bgcolor: '#fff3cd', border: '1px solid #ffeaa7', borderRadius: 2 }}>
            <Typography variant="h6" color="#856404" gutterBottom>
              {psychologist?.verificationStatus === 'rejected' ? 'Application Rejected' : 'Complete Your Profile Setup'}
            </Typography>
            <Typography variant="body2" color="#856404">
              {psychologist?.verificationStatus === 'rejected'
                ? 'Please update your information below and resubmit your application for review.'
                : 'Please complete all required fields below and submit your profile for admin approval.'
              }
            </Typography>
          </Box>
        )}
        ...
        <div className="rounded-lg shadow-md p-6 bg-white">
          <form onSubmit={handleSubmit} className="px-4">
            ...
            <Button
              ...
              sx={{ mt: 2 }}
            >
              {isPendingOrRejected ? 'Submit for Review' : 'Save Changes'}
            </Button>
          </form>
        </div>
        <ToastContainer
